this.legend_class_skill_book_item <- ::inherit("scripts/items/misc/legend_skill_book", {
	...
	function create()
	{
		...
		this.m.Description = format("Tool usage training manual, written by a master craftsman. Will teach the [color=%s]%s[/color] perk group from class perk trees to the character that uses it.", ::Const.UI.Color.NegativeValue, this.m.PerkGroupSelection);
