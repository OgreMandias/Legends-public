this.legend_profession_skill_book_item <- ::inherit("scripts/items/misc/legend_skill_book", {
	...
	function create()
	{
		...
		this.m.Description = format("Profession training manual, written by an artisan. Will teach the [color=%s]%s[/color] perk group from profession perk trees to the character that uses it.", ::Const.UI.Color.NegativeValue, this.m.PerkGroupSelection);
