this.legend_weapon_skill_book_item <- ::inherit("scripts/items/misc/legend_skill_book", {
	...
	function create()
	{
		...
		this.m.Description = format("A training manual written by a fencing master. Will teach the [color=%s]%s[/color] perk group from weapon perk trees to the character that uses it.", ::Const.UI.Color.NegativeValue, this.m.PerkGroupSelection);
